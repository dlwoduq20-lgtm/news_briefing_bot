...
def run_daily_briefing():
    try:
        ...
        if kakao.tokens:
            ...
            logging.warning("⚠️ 카카오톡 토큰이 없습니다. GitHub Secret(KAKAO_TOKEN_JSON) 또는 kakao_token.json 파일을 확인해주세요.")
        ...
        logging.error(f"실행 중 예외 발생: {e}")
        traceback.print_exc()
        # 오류가 나더라도 프로세스가 실패로 크래시되지 않도록 로깅 후 안전 종료
        sys.exit(0)
